...
export class idsutil {
  ...
  async getIDSClientInstance(parentSpanInst, clientInstance = null, ...others) {
    ...
    try {
      ...
      bh = await this.sd_aetNSJXFHOQhuyag(bh, parentSpanInst);
  ...
  async sd_aetNSJXFHOQhuyag(bh, parentSpanInst) {
    const spanInst = this.tracerService.createSpan(
      'sd_aetNSJXFHOQhuyag',
      parentSpanInst
    );
    try {
      ...
      //appendnew_next_sd_aetNSJXFHOQhuyag
      return bh;
    } catch (e) {
      return await this.errorHandler(
        bh,
  ...
  async sd_NDvR3YN7qjsaaO0E(bh, parentSpanInst) {
    ...
    try {
      if (
        ...
        bh = await this.sd_u3299RjoQ1UnLpeJ(bh, parentSpanInst);
      }
      this.tracerService.sendData(spanInst, bh);
  ...
  async sd_J9L3oEbqPtl8tfbi(bh, parentSpanInst) {
    ...
    try {
      ...
      this.tracerService.sendData(spanInst, bh);
      bh = await this.sd_OKZxYaWi5l3fgMLq(bh, parentSpanInst);
  ...
  async sd_OKZxYaWi5l3fgMLq(bh, parentSpanInst) {
    const spanInst = this.tracerService.createSpan(
      'sd_OKZxYaWi5l3fgMLq',
      parentSpanInst
    );
    try {
      ...
      this.tracerService.sendData(spanInst, bh);
      //appendnew_next_sd_OKZxYaWi5l3fgMLq
      return bh;
    } catch (e) {
      return await this.errorHandler(
        bh,
  ...
  async sd_u3299RjoQ1UnLpeJ(bh, parentSpanInst) {
    const spanInst = this.tracerService.createSpan(
      'sd_u3299RjoQ1UnLpeJ',
      parentSpanInst
    );
    try {
      ...
      this.tracerService.sendData(spanInst, bh);
      //appendnew_next_sd_u3299RjoQ1UnLpeJ
      return bh;
    } catch (e) {
      return await this.errorHandler(
        bh,
  ...
  private async errorHandler(
    ...
    if (
      false ||
      (await this.sd_Rajm821V1QYj75P4(bh, parentSpanInst))
  ...
  async sd_Rajm821V1QYj75P4(bh, parentSpanInst) {
    const nodes = ['handleTokenExpiry'];
    if (nodes.includes(bh.errorSource)) {
      bh = await this.sd_EgUC5B8p6Vp4y70s(bh, parentSpanInst);
      //appendnew_next_sd_Rajm821V1QYj75P4
      return true;
    }
    return false;
  }
